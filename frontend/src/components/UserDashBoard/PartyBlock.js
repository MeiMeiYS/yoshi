...
import { deletePartyRequest, removeFromPartiesIBelongs } from "../../store/session";
...
const PartyBlock = ({ party, sessionUser, myRequest, joinedParty }) => {
    ...
    const handleLeaveParty = (e) => {
      e.preventDefault();
      dispatch(deletePartyUser(sessionUser.id, party.id)).then(async (res) => {
        dispatch(removeFromPartiesIBelongs(party.id));
        setDisplay('none');
      })
    }
    ...
    return (
        <div className="my-user-party-container" style={{display: `${display}`}}>
                <div className="user-page-party-info">
                  <h3>{party.name}</h3>
                  <div className="party-info-group">
                    ...
                    <div className="party-platform-icon">
                      <img
                        crossOrigin="anonymous"
                        src={platformIcons[party.Videogame.platformId]}
                      ></img>
                    </div>
                    <div className="home-page-vl"></div>
                    <div className="party-member-container">
                      ...
                      <div className="party-member-count">
                        {/* need to fetch member number here */}
                      </div>
                    </div>
                  </div>
                </div>
                <div className="user-page-btn-group">
                    <div className="check-out-party-btn">
                    <NavLink exact to={`/parties/${party.id}`}>
                        Check it out
                    </NavLink>
                    </div>
                    { myRequest ?
                    ...
                    { joinedParty ?
                      (
                        ...
                        id={party.id}
                        type="button"
                        className="party-detail-remove-from-party-btn"
